...
const OnlineStore = ({ smallStats }) => {
  ...
  useEffect(() => {
    readCSVFile(onChartDataRead);
  }, []);

  function onChartDataRead (data){
    const labels = data.map(item => (item.Month));
    const shippings = data.map(item => (item.GWP));
    const profits = data.map(item => (item.Revenue));
    const taxes = data.map(item => (item.Tax));
  ...
  return (
    ...
    <Row>
      ...
      <Col lg="8" md="12" sm="12" className="mb-4">
        <SalesReport chartData={chartData} />
      </Col>
